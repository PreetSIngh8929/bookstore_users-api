...
import (
	...
	"github.com/PreetSIngh8929/bookstore_users-api/datasources/mysql/users_db"

	"github.com/PreetSIngh8929/bookstore_users-api/utils/mysql_utils"
...
const (
	...
	quertUpdateUser             = "UPDATE users SET first_name=?,last_name=?,email=? WHERE id=?;"
...
func (user *User) Save() rest_errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying toget last insert id after creating a new user", err)
		return rest_errors.NewInternalServerError("error when trying to save user", errors.New("database error"))
		// return mysql_utils.ParseError(saveErr)
	}
	user.Id = userId
...
func (user *User) Upadte() rest_errors.RestErr {
	stmt, err := users_db.Client.Prepare(quertUpdateUser)
...
func (user *User) FindByStatus(status string) ([]User, rest_errors.RestErr) {
	...
	if err != nil {
		logger.Error("error when trying tofind users by status ", err)
		return nil, rest_errors.NewInternalServerError("error when trying to find user", errors.New("database error"))
